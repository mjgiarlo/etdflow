require 'component/component_spec_helper'
require 'support/mock_depositor'

# Include shared examples
require 'component/models/depositor_interface_spec'

# So we can use fixture_file_upload
include ActionDispatch::TestProcess

describe MockDepositor do

  let(:file1) { fixture_file_upload( fixture('format_review_file_01.pdf'), 'application/pdf' ) }
  let(:file2) { fixture_file_upload( fixture('format_review_file_02.pdf'), 'application/pdf' ) }
  let(:files) { [file1, file2] }

  let!(:submission) { create :submission }

  it_behaves_like "a depositor class"

  describe "saving files" do
    describe '.save' do
      it "accepts a submission and an array of uploaded files" do
        MockDepositor.save(submission, files)
      end
    end

    describe '.saved_files' do
      context "when no files have been saved" do
        it "returns an empty array" do
          expect(MockDepositor.saved_files).to be_empty
        end
      end
      context "when a couple of files have been saved" do
        before do
          MockDepositor.save(submission, files)
        end
        it "returns an array of the file names" do
          expect(MockDepositor.saved_files).to eq [file1.original_filename, file2.original_filename]
        end
      end
    end
  end

end
